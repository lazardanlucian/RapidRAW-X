...
#[cfg_attr(not(target_os = "android"), global_allocator)]
#[cfg(not(target_os = "android"))]
...
async fn generate_ai_foreground_mask(
    ...
    Err("AI foreground mask is not available on Android yet".to_string())
}
...
async fn generate_ai_sky_mask(
    ...
    Err("AI sky mask is not available on Android yet".to_string())
}
...
async fn generate_ai_subject_mask(
    ...
    Err("AI subject mask is not available on Android yet".to_string())
}
...
async fn precompute_ai_subject_mask(
    ...
    Err("AI subject precompute is not available on Android yet".to_string())
}
...
#[cfg(target_os = "android")]
async fn save_denoised_image(
    _original_path_str: String,
    _state: tauri::State<'_, AppState>,
) -> Result<String, String> {
    Err("Denoised image saving is not available on Android yet".to_string())
}
