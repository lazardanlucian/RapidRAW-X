// lib.rs is the library entry point used for both desktop (via main.rs) and
// Android (via the #[tauri::mobile_entry_point] macro below).
...
pub fn run() {
    // tauri_plugin_single_instance is desktop-only (no Android support).
        ...
        .setup(|app| {
            ...
            *state.lens_db.lock().unwrap() = Some(lens_db);
            ...
            unsafe {
                ...
                // ORT dynamic loading is not yet set up for Android; skip on that target.
                #[cfg(not(target_os = "android"))]
                {
                let ort_library_name = {
                    ...
                    #[cfg(not(any(target_os = "windows", target_os = "linux", target_os = "macos")))] { "libonnxruntime.so" }
                };
